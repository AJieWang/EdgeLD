...
def is_partial_block(start):
    """判断是否为PABC模式的部分块(层1-6)"""
    return start in COMPUTE_PARTIAL_BLOCKS


def run_distributed_inference_PABC(namenode, round_idx):
    """运行单轮 PABC 推理（带完整时间统计）"""
    print(f"\n========== 第 {round_idx} 轮运行 (PABC模式) ==========")
    ...
    if datanode_num != 1:
        for layer_it in range(1, total_length + 1, 1):
            if layer_it > conv_length:
                ...
                print(f'全连接层 {layer_it} 耗时：{linear_end - linear_start:.3f}s')
                print(f'全连接层 {layer_it} 输出数据大小：', middle_output.size())
                ...
                if is_partial_block(start):
                    # PABC模式：DataNode一次性计算层1-6(卷积+池化)
                    end = COMPUTE_BLOCK_1_6[start]
                    cross_layer = end - start + 1 - 2 # 有两层池化
                    print(f"\n=== PABC模式-完整块: 层 {start} - {end} (包含池化层3和6) ===")
                    print(f"cross_layer: {cross_layer}")

                    divided_tensor_list, divide_record = tensor_divide_by_computing_and_network_pabc(
                        middle_output,
                    ...
                    for i in range(datanode_num):
                        thread[i] = threading.Thread(target=send_total_data,
                                                     args=(i, divided_tensor_list[i], start, end, transfer_time))
...
if __name__ == "__main__":
    ...
    print("NameNode PABC模式持久化启动，建立所有连接...")
    namenode = Network_init_namenode(namenode_num=namenode_num, datanode_num=datanode_num)
    time.sleep(2)  # 等待 DataNode 就绪
    print("所有连接已建立，开始多轮推理 (PABC模式)...")
    print("=" * 60)
    print("EdgeLD - PABC模式 (层1-6完整块计算)")
    print("层1-6在DataNode上完整执行(卷积+池化)")
    print("层7-18保持原有pooled逻辑")
    print("=" * 60)

    # 多轮推理（包含预热+有效轮次）
    for round_idx in range(1, TOTAL_ROUNDS + 1):
        run_distributed_inference_PABC(namenode, round_idx)
