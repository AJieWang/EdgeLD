...
# #############################################################################################################
# 新增：根据计算能力+网络带宽+内存约束划分区域 (V2版本)
# original_tensor默认为4维，划分后的[start, end],含start，不包含end
def tensor_divide_by_computing_network_and_memory(original_tensor, datanode_num = 1, cross_layer = 1,
                                                  computing_power = [], computing_a = [], computing_b = [], 
                                                  network_state = [], memory_state = [], c_out = 0):
    step = 1
    divided_tensor = []
    divide_record = np.zeros((datanode_num, 2), dtype=np.int)
    input_param = []
    ...
    if datanode_num == 1:
        ...
        # 计算输入数据大小 (float32, 4 bytes)
        input_tensor_size = input_number * c_in * height * width * 4
        ...
        # 时间开销
        prediction_time = []
        # 内存约束因子 (1.0 = 正常, <1.0 = 减少分配)
        memory_factor = []
        
        for i in range(datanode_num):
            length.append(0)
            prediction_time.append(0)
            # 检查每个节点的内存是否足以容纳输入数据（预留50%余量）
            available_memory = memory_state[i] * 0.5
            if input_tensor_size > available_memory:
                # 内存不足，减小分配比例
                memory_factor.append(available_memory / input_tensor_size)
                print(f"DataNode {i}: 内存不足 (需要 {input_tensor_size/1e6:.1f}MB, 可用 {available_memory/1e6:.1f}MB), 分配因子 {memory_factor[i]:.2f}")
            else:
                memory_factor.append(1.0)
        
        # 根据计算能力和内存约束计算初始划分
        for it in range(datanode_num):
            # 基础划分比例 * 内存因子
            adjusted_power = computing_power[it] * memory_factor[it]
            length[it] = int(sum_computing_power[it+1]/total_computing_power * width) - \
                         int(sum_computing_power[it] / total_computing_power * width)
        ...
        while(True):
            ...
            # 检查内存约束：如果某个节点内存不足，不再增加其分配
            can_increase = True
            if index_min < datanode_num:
                # 计算增加后的数据大小
                new_length = length[index_min] + step
                new_tensor_size = input_number * c_in * height * (new_length / width) * width * 4
                available_memory = memory_state[index_min] * 0.5
                if new_tensor_size > available_memory:
                    can_increase = False
            ...
            prediction_time[index_max] = get_prediction_time(datanode_num = datanode_num, index = index_max, 
                                                            ...
                                                            network_state = network_state, input_param = input_param, c_out = c_out)
